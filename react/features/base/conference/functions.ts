...
function safeStartCase(s = '') {
    // Remove file extensions like .html to prevent them from appearing in titles
    const cleanString = `${s}`.replace(/\.[a-zA-Z]+$/, '').replace(/['\u2019]/g, '');
    return _.words(cleanString).reduce(
