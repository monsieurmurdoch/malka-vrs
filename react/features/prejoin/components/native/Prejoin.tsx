...
const Prejoin: React.FC<IPrejoinProps> = ({ navigation }: IPrejoinProps) => {
    ...
    // VRS functionality
    const userRole = isClient() ? 'client' : 'interpreter';
    ...
    // VRS-specific handlers
    const handleRequestInterpreter = useCallback(() => {
        if (userRole === 'client' && !isRequestingInterpreter) {
            setIsRequestingInterpreter(true);
            queueService.requestInterpreter('any', displayName);
        }
    }, [ userRole, isRequestingInterpreter, displayName ]);

    const handleCancelRequest = useCallback(() => {
        if (isRequestingInterpreter) {
    ...
    useEffect(() => {
        ...
        if (userRole === 'client') {
            ...
            const handleRequestQueued = () => {
                // Request was queued - keep the requesting state
            };
            ...
            return () => {
                ...
                queueService.off('requestCancelled', handleRequestCancelled);
            };
        }
        ...
        <JitsiScreen
            ...
            <View style = { contentContainerStyles as ViewStyle }>
                    ...
                    type = { TERTIARY } />
                {
                    userRole === 'client' && (
                        <Button
                            accessibilityLabel = { isRequestingInterpreter ? 'Cancel interpreter request' : 'Request interpreter' }
                            disabled = { showDisplayNameError }
                            label = { isRequestingInterpreter ? 'Cancel Interpreter Request' : 'Request Interpreter' }
                            onClick = { isJoining ? undefined : (isRequestingInterpreter ? handleCancelRequest : handleRequestInterpreter) }
                            style = { styles.joinButton }
                            type = { isRequestingInterpreter ? TERTIARY : PRIMARY } />
                    )
                }
            </View>
        </JitsiScreen>
