...
import PhoneNumpad from '../../../interpreter-phone/components/PhoneNumpad';
import { twilioVoiceService } from '../../../interpreter-phone/services/TwilioVoiceService';
import QueueStatusIndicator from '../../../interpreter-queue/components/web/QueueStatusIndicator';
...
class Conference extends AbstractConference<IProps, any> {
    ...
    state = {
        callStatus: 'idle' as 'idle' | 'dialing' | 'ringing' | 'connected' | 'ended',
        callDuration: ''
    };
    ...
    constructor(props: IProps) {
        ...
        // Bind VRS phone handlers
        this._handleMakeCall = this._handleMakeCall.bind(this);
        this._handleHangupCall = this._handleHangupCall.bind(this);
    ...
    componentDidMount() {
        ...
        // Set up Twilio voice service event listeners for interpreters
        if (isInterpreter()) {
            twilioVoiceService.on('statusChange', this._handleStatusChange);
            twilioVoiceService.on('durationUpdate', this._handleDurationUpdate);
        }
    }
    ...
    componentWillUnmount() {
        ...
        // Clean up Twilio voice service listeners
        if (isInterpreter()) {
            twilioVoiceService.off('statusChange', this._handleStatusChange);
            twilioVoiceService.off('durationUpdate', this._handleDurationUpdate);
    ...
    render() {
        ...
        const useVrsLayout = !_showPrejoin && !_showLobby && isVrsSession(_roomName);
            ...
            <div
                ...
                <div
                    ...
                    <div
                        ...
                        {useVrsLayout ? <VRSLayout /> : <LargeVideo />}
                        {
                            _showPrejoin || _showLobby || useVrsLayout || (<>
                                <StageFilmstrip />
                ...
                {/* VRS Numpad for interpreters only - displayed in meeting room */}
                {isInterpreter() && !_showPrejoin && !_showLobby && (
                    <PhoneNumpad
                        onCall = { this._handleMakeCall }
                        onHangup = { this._handleHangupCall }
                        callStatus = { this.state.callStatus }
                        callDuration = { this.state.callDuration }
                    />
                )}

                {/* VRS Components - shown during active meeting */}
                {!_showPrejoin && !_showLobby && (
                    <>
                        {/* Client queue status indicator - shows when waiting for interpreter */}
                        <QueueStatusIndicator />

                        {/* Interpreter request notifications - shown to interpreters in-meeting */}
                        <InMeetingRequestNotification />

                        {/* Interpreter status toggle - allows interpreters to go available/busy */}
                        {isInterpreter() && <InterpreterStatusToggle />}
    ...
    /**
     * Handler for making VRS phone calls
     */
    async _handleMakeCall(phoneNumber: string) {
        console.log(`Making VRS call to: ${phoneNumber}`);
        const success = await twilioVoiceService.makeCall({
            phoneNumber,
            sessionId: `vrs-${Date.now()}`
        });
    ...
    /**
     * Handler for hanging up VRS phone calls
     */
    async _handleHangupCall() {
        console.log('Ending VRS call');
        await twilioVoiceService.hangupCall();
    }

    /**
     * Handler for Twilio call status changes
     */
    _handleStatusChange(callStatus: any) {
        this.setState({ callStatus: callStatus.status });
    }

    /**
     * Handler for call duration updates
     */
    _handleDurationUpdate(duration: string) {
