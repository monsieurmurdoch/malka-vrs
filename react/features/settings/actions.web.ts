...
export function submitProfileTab(newState: any) {
    return (dispatch: IStore['dispatch'], getState: IStore['getState']) => {
        ...
        if (newState.currentLanguage && newState.currentLanguage !== currentState.currentLanguage) {
            void i18next.changeLanguage(newState.currentLanguage);
            document.documentElement.dir = newState.currentLanguage === 'ar' ? 'rtl' : 'ltr';
            document.documentElement.lang = newState.currentLanguage;

            try {
                localStorage.setItem('vrs_preferred_language', newState.currentLanguage);
            } catch (error) {
                // Ignore persistence failures and keep the in-memory language change.
            }
        }
    };
}
