...
### Admin Refinement & UX Polish
- [x] Expanded interpreter admin profile into a full CRM-style record: password reset, company/alternate email, schedule/billing/payment notes, manager comments, language permissions, and VRI/VRS queues
- [x] Added corporate client account creation/editing from tenant admin and superadmin contexts
- [x] Made interpreter/client/account CSV exports available from roster tables
- [x] Clarified admin dashboard labels: Available Interpreters are staff ready for matching; Waiting Client Requests are clients currently waiting in queue
- [x] Tightened admin live refresh for interpreter availability and queue-state changes so manual refresh is not normally needed
- [x] Replaced CRM note fields with dedicated schedule, billing, payout, utilization, and manager-note tables
...
### Billing, Payouts & Utilization Backend
- [x] VRS CDR immutability: append-only after call end
- [x] VRI corporate account management
...
- [x] Expanded Stripe webhook handling for disputes, refunds, customer/subscription changes, and idempotent event recording
...
### Mobile Parity Foundations
- [x] Client mobile invite flow support once VRI session invite model is built
