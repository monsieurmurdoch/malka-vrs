    ...
    async initialize(): Promise<void> {
        try {
            ...
            if (!bucketExists) {
                ...
                log.info({ bucket: this.config.bucket }, 'storage_bucket_created');
            }

            this.initialized = true;
            log.info({ bucket: this.config.bucket, endpoint }, 'storage_initialized');
        } catch (err) {
            log.error({ err }, 'storage_initialization_failed');
            throw err;
        }
    }
