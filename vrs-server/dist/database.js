...
async function getQueueRequests(status = 'waiting') {
    const requests = await runQuery('SELECT * FROM queue_requests WHERE status = $1 ORDER BY position', [status]);
...
async function createInterpreterShift({ interpreterId, date, startTime }) {
    const id = (0, uuid_1.v4)();
    await runInsert(`INSERT INTO interpreter_shifts (id, interpreter_id, date, start_time)
         VALUES ($1, $2, $3, $4)
         ON CONFLICT (interpreter_id, date)
         DO UPDATE SET start_time = EXCLUDED.start_time, id = EXCLUDED.id`, [id, interpreterId, date, startTime]);
    return { id, date };
}
