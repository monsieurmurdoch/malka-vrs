...
```bash
LINEAR_API_KEY=... mix symphony.run --workflow /path/to/malka-vrs/WORKFLOW.md
```

If your local Symphony runner exposes a different CLI entrypoint, use that runner's equivalent `--workflow /path/to/malka-vrs/WORKFLOW.md` option. The workflow file is self-contained: it selects Linear, clones this repo, prepares the workspace, and launches Codex with `codex ... app-server`.
